...
class Sale extends CI_Controller {
    ...
    /* ----------------------Sale Start-------------------------- */
    public function remove_numbers($string) {
    ...
    public function printSale($sale_id)
     {
         ...
         try {
             ...
             if ($this->session->userdata['tax_registration_no'] && $this->session->userdata('collect_tax')=='Yes'):
                 $printer -> text('Tax Registration No: '.$this->session->userdata('tax_registration_no')."\n");
             endif;
             $printer -> feed();
             /* Title of receipt */
             ...
             $printer -> text('INVOICE NUMB.: '.$order_type.' '.$data->sale_no."\n");
             // data sale
             $printer -> text('SALES ASSOCIATE: '.$data->user_name."\n");
             ...
             $printer -> setEmphasis(true);
             $printer -> text(new item('Items', $this->session->userdata('currency')));
             if (isset($data->items)) {
                 /* Items */
                 $printer -> setEmphasis(false);
                 foreach ($data->items as $row) {
                     $printer -> text(new item($row->menu_name, ''));
                     $printer -> text(new item($row->qty.'X'.'@ 1/ '.number_format($row->menu_unit_price,0,",","."), number_format($row->menu_price_without_discount,0,",",".")));
                 
                 if(count($row->modifiers)>0) {
                     $printer -> text('Modifier:');
                     ...
                     $modifier_name = $this->remove_numbers($modifier_name);
                     $printer -> text(new item($modifier_name, number_format($modifier_price,0, ",", ".")));
                 }
             }
             ...
             if ($this->session->userdata('collect_tax')=='Yes' && $sale_object->sale_vat_objects!=NULL):
                 foreach(json_decode($data->sale_vat_objects) as $single_tax) {
                     $printer -> text(new item($single_tax->tax_field_type, $data->tax_field_amount));        
                 }
             endif;
             $printer -> setEmphasis(true);
             $printer -> text(new item('Sub Total', number_format($data->sub_total,0,",",".")));
             $printer -> feed();
             $printer -> setEmphasis(false);
             $printer -> text(new item('Disc Amt (%)', number_format($data->total_item_discount_amount,0,",",".")));
             $printer -> text(new item('Service/Delivery Charge', number_format($data->delivery_charge,0,",",".")));
             $printer -> setEmphasis(true);
             $printer -> feed();
             
             // $printer -> selectPrintMode(Printer::MODE_DOUBLE_WIDTH);
             $printer -> text(new item('Grand Total', number_format($data->total_payable,0,",",".")));
             $printer -> text(new item('Cash', number_format($given_amount,0,",",".")));
             $printer -> text(new item('Change Amount', number_format($change_amount,0,",",".")));
             // $printer -> selectPrintMode();
             $printer -> setEmphasis(false);
             ...
             $printer -> feed(2);
             $printer -> setJustification(Printer::JUSTIFY_CENTER);
             $printer -> text($data->invoice_footer."\n");
             $printer -> feed(2);
             $printer -> text(date($this->session->userdata('date_format'), strtotime($data->sale_date)) .' '. date('H:i',strtotime($data->order_time)) . "\n");
    ...
    public function POS($encrypted_id = "") {
        ...
        $data['menu_discount'] = $this->Sale_model->getAllDiscount();
        $data['menu_discount_allitem'] = $this->Sale_model->getAllItemDiscount();
        $data['menu_delivery'] = $this->Sale_model->getAllDelivery();
        ...
        $data['master_menu'] = 'sales';
        $data['sub_menu'] = '';
        $data['data_sales'] = $this->get_all_information_of_a_sale("314");
    ...
    public function Save() {
        ...
        $data['given_amount'] = $this->input->get('given_amount');
        $data['change_amount'] = $this->input->get('change_amount');
    ...
    public function update_order_status_ajax()
    {
        ...
        if($close_order=='true'){
            ...
            if($is_just_cloase){
                ...
                $order_status = array('paid_amount' =>  $paid_amount, 'due_amount' => $due_amount, 'order_status' => 3,'payment_method_id'=>$payment_method_type,'given_amount'=>$given_amount, 'change_amount' =>$change_amount,'close_time'=>date('H:i:s'));    
            }

        }else{
            $order_status = array('paid_amount' => $paid_amount,'due_amount' => $due_amount,'order_status' => 2,'payment_method_id'=>$payment_method_type,'given_amount'=>$given_amount,'change_amount' =>$change_amount);
        }
        $this->printSale($sale_id); //print invoice
        $this->db->where('id', $sale_id);
        $this->db->update('tbl_sales', $order_status);
        echo $sale_id;
    }
    ...
    public function printKOT($temp_kot_id)
    {
        try {
            $connector = new NetworkPrintConnector("192.168.0.22", 9100);
            ...
            $data = $this->Sale_model->get_temp_kot($temp_kot_id);
            $kot_info = $data->temp_kot_info;
            $kot_info = json_decode($kot_info);
            // $data = $this->get_all_information_of_a_sale($sale_id);

   
            $printer -> text('INVOICE NUMB.: '.$kot_info->order_number."\n");
            // data sale
            // $printer -> text('SALES ASSOCIATE: '.$kot_info->user_name."\n");
            $printer -> text('CUSTOMER: '.$kot_info->customer_name."\n");
            $printer -> text('KOT WAITER: '.$kot_info->waiter_name."\n");
            $printer -> text('ORDER TYPE: '.$kot_info->order_type."\n");

            // print_r($data->items);die;

            $printer -> setJustification(Printer::JUSTIFY_LEFT);
            $printer -> setEmphasis(true);
            $printer -> text(new item('SN | Items', 'QTY'));
            ...
            if (isset($kot_info->items)) {
                ...
                foreach ($kot_info->items as $row) {
                    ...
                    $printer -> text(new item($i.' | '.$row->kot_item_name, 'X'.$row->kot_item_qty));

                    if($row->modifiers) {
                        $printer -> text('Modifier:');
                        $printer -> text($row->modifiers);
                    }
    ...
    public function printKOT2($sale_id)
    {
        try {
            $connector = new NetworkPrintConnector("192.168.0.22", 9100);
            ...
            if($data->order_type == 1){
                ...
                $order_type = 'B';
                $order_type2 = 'Take Away';
            }elseif($data->order_type == 3){
                $order_type = 'C';
                $order_type2 = 'Delivery';
            }
            // $data = $this->get_all_information_of_a_sale($sale_id);
            ...
            // $printer -> text('SALES ASSOCIATE: '.$kot_info->user_name."\n");
            $printer -> text('CUSTOMER: '.$data->customer_name."\n");
            $printer -> text('KOT WAITER: '.$data->waiter_name."\n");
            $printer -> text('ORDER TYPE: '.$order_type2."\n");
            ...
            if (isset($data->items)) {
                ...
                foreach ($data->items as $row) {
                    $printer -> text(new item($i.' | '.$row->menu_name, 'X'.$row->qty));
