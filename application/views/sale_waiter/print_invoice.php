...
<body>
    <div id="wrapper">
      ...
      function slip(){
            ...
            prn += PrnBoldOn+'Invoice No.<?= $order_type.' '.$sale_object->sale_no ?>'+PrnBoldOff+LF;
            prn += 'Date :<?= date($this->session->userdata('date_format'), strtotime($sale_object->sale_date)); ?> <?= date('H:i',strtotime($sale_object->order_time)) ?>' +LF+LF;
            prn += PrnAlignRight;
            prn += 'Sales Associate: <?php echo $sale_object->user_name; ?>'+LF;
            prn += 'Customer : <?php echo "$sale_object->customer_name"; ?>'+LF;
            ...
            if (isset($sale_object->items)) {
                ...
                foreach ($sale_object->items as $row) {
                    ...
                    prn += PrnAlignLeft;
                    prn += '<?php echo $row->code; ?>'+LF;

                    prn += '--------------------------------'+LF;

                    prn += PrnAlignLeft+ '<?php echo $row->menu_name; ?> '+LF;
                    prn += PrnAlignRight+ '<?php echo "$row->qty X $row->menu_unit_price"; ?>'+LF;
                    ...
                    <?php if(count($row->modifiers)>0){ ?>
                            ...
                            foreach($row->modifiers as $modifier){
                                if($l==count($row->modifiers)){
                                  ?>
                                    prn += PrnAlignRight+ '<?php echo "$modifier->name"; ?>'+LF;
                                    <?php
                                }else{
                                  ?>
                                  prn += PrnAlignRight+ '<?php echo "$modifier->name"; ?> ,'+LF;
                        ...
                        prn += PrnAlignRight+ '<?php echo $this->session->userdata('currency') . " " . $modifier_price; ?>'+LF;
            ...
            prn += '--------------------------------'+LF;
            prn += PrnAlignLeft+PrnBoldOn+'Total Item <?= $totalItems ?>'+PrnBoldOff+LF;
            prn += PrnAlignRight+PrnBoldOn+'Sub Total : <?php echo $this->session->userdata('currency') . " " . number_format($sale_object->sub_total, 2); ?>'+PrnBoldOff+LF;
            prn += PrnAlignRight+PrnBoldOn+'Disc Amt(%) <?php echo $this->session->userdata('currency') . " " . $sale_object->total_discount_amount; ?>'+PrnBoldOff+LF;
            prn += PrnAlignRight+PrnBoldOn+'Grand Total <?php echo $this->session->userdata('currency') . " " . number_format($sale_object->total_payable, 2); ?>'+PrnBoldOff+LF;
            prn += PrnAlignRight+PrnBoldOn+'Cash <?php echo $this->session->userdata('currency') . " " . number_format($sale_object->given_amount, 2); ?>'+PrnBoldOff+LF;
            prn += PrnAlignRight+PrnBoldOn+'Change <?php echo $this->session->userdata('currency') . " " . number_format($sale_object->change_amount, 2); ?>'+PrnBoldOff+LF;
