		...
		if ($('#single_order_' + sale_id).attr('data-order-type') == 'Dine In') {
			if ($('#single_order_' + sale_id + ' .items_holder .single_item[data-selected=selected]').length > 0) {
				...
				$('#single_order_' + sale_id + ' .items_holder .single_item[data-selected=selected]').each(function (i, obj) {
					arrPrev.push($(this).attr('id').substr(15));
					if (j == total_items) {
				...
				if (previous_id != '') {
					...
					$.ajax({
						...
						success: function (response) {
							console.log(arrPrev.length);
							print_kitchen(arrPrev);

							swal({
								title: 'Alert',
								text: "Cooking Started!!" + previous_id,
								confirmButtonColor: '#b6d6f6'
							});
...
function print_kitchen(previous_id) {

	if (previous_id.length > 0) {
		var prev = '';
		var i;
		var j = 1;
		for (i = 0; i < previous_id.length; i++) {
			if (j == previous_id.length) {
				prev += previous_id[i];
			} else {
				prev += previous_id[i] + ',';
			}
		...
		let newWindow = open("getKitchenPrint/?id=" + prev, 'Print Kitchen', 'width=450,height=550')
		newWindow.focus();
