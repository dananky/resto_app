...
$(document).ready(function() {
    ...
    $(document).on('click', '.single_hold_sale', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#delivery_order_hold").html(response.devlivery_order);
                $("#tax_change_hold").html(response.tax_charge);
    ...
    $(document).on('click', '.single_last_ten_sale', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("delivery_order_last_10").html(response.delivery_order);
                $("#tax_charge_last_10").html(response.tax_charge);
            ...
            if (r == true) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $("#delivery_order_hold").html('0.00');
                        $("#tax_charge_hold").html('0.00');
    ...
    $('#given_amount_input').on('input', function(e) {
        // var paid_amount = $('#pay_amount_invoice_input').val();
        // var given_amount = $('#given_amount_input').val();

        // var submit_given = ($(this).val() < paid_amount) ? 

        // if (given_amount < paid_amount) {
        //     $('#finalize_order_button').prop('disabled', 'disabled');
        //     return false;
        // } 
    ...
    $('#pay_amount_invoice_input').on('keypress', function(e) {
        //if the letter is not digit then display error and don't type anything
        if (e.which != 190 && e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
            $(this).val('');
        }
    ...
    // 	$('#button_category_show').on('change',function(){

    // 		var str = $('option:selected',this).show();
    // 		var res = str.substr(16);
    // 		$("#searched_item_found").remove();
    // 		$('.specific_category_items_holder').hide('1000');
    // 		$('#category_'+res).show('1000');
    ...
    $("#edit_all_diskon").on("click", function() {


        $("#item_all_diskon .section3").empty();
        $("#item_all_diskon .section3").prepend();
        $('#item_all_diskon').fadeIn('500');
    });



    $(document).on('click', '#close', function() {
        $("#item_all_diskon").hide();
    });


    $("#modal_all_discount").on("click", function() {

        var item_vat_percentage = $(this).attr("data-amount");
        item_vat_percentage = (item_vat_percentage == "") ? '0.00' : item_vat_percentage;

        $("#sub_total_discount").val(item_vat_percentage);
        // $("#all_items_discount").html(item_vat_percentage);
        $("#item_all_diskon").hide();
    ...
    $(document).on('click', '.edit_diskon', function() {
        ...
        var single_order_element_object = $(this).parent().parent().parent();
        var row_number = $(this).parent().parent().parent().attr('data-single-order-row-no');
        var menu_id = $(this).attr('id').substr(12);
        var item_name = single_order_element_object.find('#item_name_table_' + menu_id).html();
        var item_price = single_order_element_object.find('#item_price_table_' + menu_id).html();

        var item_vat_percentage = single_order_element_object.find('#item_vat_percentage_table' + menu_id).html();
        var item_discount_input_value = document.getElementById('modal_discount').innerHTML;
        ...
        $("#item_modal_diskon .section3").empty();
        $("#item_modal_diskon .section3").prepend();
        $('#item_modal_diskon').fadeIn('500');

    });


    $(document).on('click', '#modal_discount', function() {

        // var price_value = document.getElementById('modal_price').innerHTML;
        // var amount_value = document.getElementById('modal_discount').innerHTML;
        // console.log (price_value);
        // console.log (amount_value);


        // var discount_input_value = document.getElementById('modal_discount').innerHTML;
        // console.log(discount_input_value);
        // $("#item_modal_diskon").hide();
        ...
        var item_vat_percentage = $(this).attr("data-amount")
        // var item_vat_percentage = $(this).attr("data-amount") + $(this).attr("data-price");
        item_vat_percentage = (item_vat_percentage == "") ? '0.00' : item_vat_percentage;

        var item_discount_amount = (parseFloat($('#modal_discount_amount').html())).toFixed(2);

        // var discount_input_value = $(this).attr("data-amount");
        var discount_input_value = $(this).attr("data-amount") + $(this).attr("data-price");


        if (discount_price == 'Rp') {
            var myString = discount_price.substring(2, discount_price.length);
            var discount_input_value = $(this).attr("data-amount") + myString;
        } else {
            var discount_input_value = $(this).attr("data-amount") + $(this).attr("data-price");
        ...
        // } else if(item_vat_percentage == item_vat_percentage) {
        ...
        //get item/menu total price from modal
        // var item_total_price = item_price - item_vat_amount_for_unit_item;
        ...
        // draw_table_for_order += '<div style="margin-left: 379px;" class="single_order_column forth_column fix"><input type="text" name="" placeholder="Amt or %" class="special_textbox" id="percentage_table_' + item_id + '" value="' + discount_input_value + '" disabled></div>';
        ...
        // $("#item_modal_diskon").hide();
        reset_on_modal_close_or_add_discount();
        // return false;
        //do calculation on table
    });
    ...
    $(document).on('click', '.delivey_order', function() {

        $("#item_modal_delivery .section3").empty();
        $("#item_modal_delivery .section3").prepend();
        $('#item_modal_delivery').fadeIn('500');
    ...
    $(document).on('click', '#modal_delivery', function() {

        var item_vat_percentage = $(this).attr("data-price");
        item_vat_percentage = (item_vat_percentage == "") ? '0.00' : item_vat_percentage;

        $("#delivery_order").val(item_vat_percentage);
        $("#item_modal_delivery").hide();
    ...
    $(document).ready(function() {
        $('#button_category_show_all').on('click change touchstart', function(e) {
            e.preventDefault();
            ...
            $("#searched_item_found").remove();
            $('.specific_category_items_holder').hide('1000');
            $(".category_items").append(searched_category_items_to_show);
        })

        // 		$(".category_items"+res).show('1000');
    ...
    $('#sub_total_discount').on('keypress', function(e) {
        //if the letter is not digit then display error and don't type anything
        if (e.which != 190 && e.which != 16 && e.which != 53 && e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
            $(this).val('');
        }
    ...
    $('#delivery_order').on('keyup', function(e) {
        do_addition_of_item_and_modifiers_price();
    })

    $('#tax_charge').on('keyup', function(e) {
        do_addition_of_item_and_modifiers_price();
    });
    ...
    $('#place_order_operation').on('click', function() {
        ...
        console.log('delivery_change', delivery_change);


        var delivery_order2 = ($('#delivery_order').val() != "") ? parseFloat($('#delivery_order').val()).toFixed(2) : parseFloat(0).toFixed(2);
        var delivery_order = delivery_order2 * sub_total / 100;

        var tax_charge = ($('#tax_charge').val() != "") ? parseFloat($('#tax_charge').val()).toFixed(2) : parseFloat(0).toFixed(2);
        ...
        if (selected_order_type_object.length > 0) {
            ...
            order_info += '"delivery_order":"' + delivery_order + '",';
            order_info += '"tax_charge":"' + tax_charge + '",';
    ...
    $('#finalize_order_button').on('click', function() {
        ...
        if ($('.holder .order_details > .single_order[data-selected=selected]').length > 0) {
            var sale_id = $('.holder .order_details .single_order[data-selected=selected]').attr('id').substr(6);
            var payment_method_type = $('#finalize_order_payment_method').val();
            var paid_amount = $('#pay_amount_invoice_input').val();
            var due_amount = $('#due_amount_invoice_input').val();
            var given_amount = $('#given_amount_input').val();
            var change_amount = $('#change_amount_input').val();
            var invoice_create_type = $('#finalize_update_type').html();
            console.log(paid_amount);
            if (payment_method_type == "") {
                $('#finalize_order_payment_method').css('border', '1px solid red');
                return false;
            } else {
                $('#finalize_order_payment_method').css('border', '1px solid #b6b6b3');
            }

            if (given_amount <= paid_amount - 1) {
                $('#given_amount_input').css('border', '1px solid red');
                return false;
            } else {
                $('#given_amount_input').css('border', '1px solid #b6b6b3');
            }

            print_invoice_and_close(sale_id, payment_method_type, invoice_create_type, paid_amount, due_amount, given_amount, change_amount);
            reset_finalize_modal();
        } else {
            swal({
                title: warning + "!",
...
function show_all_items() {
    ...
    $('#refresh_order').on('click', function() {
        ...
        // 		$('#order_details_holder').fadeOut('500');
    });
    // 	$('#order_details_holder').on('click',function(){
    // 		$('#show_tables_modal').fadeOut('500');
    // 	});
...
function do_addition_of_item_and_modifiers_price() {
    ...
    console.log('sub total', total_of_all_items_and_modifiers);

    //get the value of the delivery charge amount

    //var delivery_charge_amount = ($('#delivery_charge').val() != "") ? $('#delivery_charge').val() : 0;

    var delivery_charge_amount = $('#delivery_charge').val() * total_of_all_items_and_modifiers / 100;

    //check wether value is valid or not
    remove_last_two_digit_without_percentage(delivery_charge_amount, $('#delivery_charge'));

    
    var delivery_order_amount = $('#delivery_order').val() * total_of_all_items_and_modifiers / 100;
    remove_last_two_digit_with_percentage(delivery_order_amount, $('#delivery_order'));

    //get the value of the delivery charge amount
    //var tax_charge_amount = ($('#tax_charge').val() != "") ? $('#tax_charge').val() : 0;

    var tax_charge_amount = $('#tax_charge').val() * total_of_all_items_and_modifiers / 100;


    //check wether value is valid or not
    remove_last_two_digit_without_percentage(tax_charge_amount, $('#tax_charge'));
    ...
    // //get the value of the delivery charge amount
    // var delivery_order_amount = ($('#delivery_order').val() != "") ? $('#delivery_order').val() : 0;
    // //check wether value is valid or not
    // remove_last_two_digit_without_percentage(delivery_order_amount, $('#delivery_order'));
    ...
    console.log('diskon', discounted_sub_total_amount);
    ...
    var total_payable = (parseFloat(discounted_sub_total_amount) + parseFloat(total_vat_amount) + parseFloat(delivery_charge_amount) +
        parseFloat(tax_charge_amount) + parseFloat(delivery_order_amount)).toFixed(2);
    
    // var total_payable = (parseFloat(discounted_sub_total_amount) + parseFloat(total_vat_amount) + parseFloat(delivery_charge_amount) + parseFloat(tax_charge_amount) + parseFloat(delivery_order_amount)).toFixed(2);
    console.log('total', total_payable);
...
function reset_on_modal_close_or_add_discount() {
    $('#item_modal_diskon').fadeOut('500');
    $('#modal_discount').val('');
    $('#modal_item_row').html('0');
    $('#modal_discount_amount').html('0');
}
...
function reset_customer_waiter_to_default() {
    ...
    var wid = $('#select_waiter').attr('value');
...
function print_invoice_and_close(sale_id, payment_method_type, invoice_create_type, paid_amount, due_amount, given_amount, change_amount) {

    if (invoice_create_type == 1) { //if type is 1 then update order status to invoiced order, and update payment method type
        update_order_status_to_invoiced(sale_id, payment_method_type, paid_amount, due_amount, given_amount, change_amount);
        update_order_status_to_invoiced2(sale_id, payment_method_type, paid_amount, due_amount, given_amount, change_amount);
    } else if (invoice_create_type == 2) { //then change order status to close, close time update, payment method type update, 
        close_order(sale_id, payment_method_type, paid_amount, due_amount, given_amount, change_amount);
        close_order2(sale_id, payment_method_type, paid_amount, due_amount, given_amount, change_amount);
    }
    print_invoice(sale_id);
}
...
function update_order_status_to_invoiced(sale_id, payment_method_type, paid_amount, due_amount, given_amount, change_amount) {
    $.ajax({
...
function update_order_status_to_invoiced2(sale_id, payment_method_type, paid_amount, due_amount, given_amount, change_amount) {
    $.ajax({
        url: base_url + "Sale/printSale",
        method: "POST",
        data: {
            sale_id: sale_id,
            close_order: false,
...
function close_order(sale_id, payment_method_type, paid_amount, due_amount, given_amount, change_amount) {
    $.ajax({
        url: base_url + "Sale/update_order_status_ajax",
        method: "POST",
        data: {
...
function close_order2(sale_id, payment_method_type, paid_amount, due_amount, given_amount, change_amount) {
    $.ajax({
        url: base_url + "Sale/printSale",
        method: "POST",
        data: {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var key in response.items) {
                ...
                draw_table_for_order += '<div class="single_order_column second_column fix"> <span id="item_price_table_' + this_item.food_menu_id + '">' + this_item.menu_unit_price + '</span></div>';
                ...
                draw_table_for_order += '<div class="single_order_column fifth_column fix"> <span id="item_total_price_table_' + this_item.food_menu_id + '">' + this_item.menu_price_with_discount + '</span></div>';
                draw_table_for_order += '</div>';
                if (selected_modifiers != "") {
                    draw_table_for_order += '<div class="second_portion fix">';
                    ...
                    draw_table_for_order += '<div class="single_order_column fifth_column fix"> <span id="item_modifiers_price_table_' + this_item.food_menu_id + '">' + modifiers_price + '</span></div>';
                    draw_table_for_order += '</div>';
                }
                if (this_item.menu_note != "") {
                    draw_table_for_order += '<div class="third_portion fix">';
            ...
            $("#tax_charge").val(response.tax_charge);
...
function get_details_of_a_particular_order_for_modal(sale_id) {
    $.ajax({
        ...
        success: function(response) {
            ...
            $("#delivery_order_order_details").html(response.order_charge);
            // $("#delivery_order_details").html(response.delivery_order);
...
function reset_last_10_sales_modal() {
    ...
    $("#delivery_order_last_10").html('0.00');
    $("#tax_charge_last_10").html('0.00');
...
function reset_finalize_modal() {
    ...
    $('#finalize_order_payment_method').val("");
    $('#finalize_update_type').html('');
    $('#pay_amount_invoice_input').val('');
    $('#due_amount_invoice_input').val('');
    $('#finalize_order_payment_method').css('border', '1px solid #B5D6F6');
}
